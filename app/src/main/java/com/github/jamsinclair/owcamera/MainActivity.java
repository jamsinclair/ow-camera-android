...
import com.github.jamsinclair.owcamera.CameraController.CameraController;
import com.github.jamsinclair.owcamera.CameraController.CameraControllerManager2;
import com.github.jamsinclair.owcamera.Preview.Preview;
import com.github.jamsinclair.owcamera.UI.FolderChooserDialog;
import com.github.jamsinclair.owcamera.UI.MainUI;
import com.github.jamsinclair.owcamera.PebbleHelper.PebbleHelper;
...
/** The main Activity for OW Camera for Pebble.
 */
public class MainActivity extends Activity implements AudioListener.AudioListenerCallback {
    ...
    private boolean screen_is_locked; // whether screen is "locked" - this is OW Camera for Pebble's own lock to guard against accidental presses, not the standard Android lock
	 ...
	 * This method should be called when OW Camera for Pebble is run for the very first time after installation,
	...
	private void setModeFromIntents(Bundle savedInstanceState) {
        ...
        if( MediaStore.INTENT_ACTION_VIDEO_CAMERA.equals(action) || MediaStore.ACTION_VIDEO_CAPTURE.equals(action) ) {
			...
			if( MyDebug.LOG )
				Log.d(TAG, "launching from quick settings tile for OW Camera for Pebble: photo mode");
			applicationInterface.setVideoPref(false);
		}
		else if( MyTileServiceVideo.TILE_ID.equals(action) ) {
			if( MyDebug.LOG )
				Log.d(TAG, "launching from quick settings tile for OW Camera for Pebble: video mode");
			applicationInterface.setVideoPref(true);
		}
		else if( MyTileServiceFrontCamera.TILE_ID.equals(action) ) {
			if( MyDebug.LOG )
				Log.d(TAG, "launching from quick settings tile for OW Camera for Pebble: selfie mode");
    ...
    public void setWindowFlagsForCamera() {
		...
		if( sharedPreferences.getBoolean(PreferenceKeys.getShowWhenLockedPreferenceKey(), true) ) {
	        ...
	        // keep OW Camera for Pebble on top of screen-lock (will still need to unlock when going to gallery or settings)
    ...
    /** Opens OW Camera for Pebble's own (non-Storage Access Framework) dialog to select a folder.
     */
    ...
    /** Lock the screen - this is OW Camera for Pebble's own lock to guard against accidental presses,
		...
		switch( requestCode ) {
	        case MY_PERMISSIONS_REQUEST_CAMERA:
	        {
	            ...
	            if( grantResults.length > 0
	            	...
	            	// OW Camera for Pebble doesn't need to do anything: the camera will remain closed
	            }
	            return;
	        }
