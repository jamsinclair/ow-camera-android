...
/** Handles the OW Camera for Pebble lock screen widget. Lock screen widgets are no
 *  longer supported in Android 5 onwards (instead OW Camera for Pebble can be launched
...
public class MyWidgetProvider extends AppWidgetProvider {
    ...
    public void onUpdate(Context context, AppWidgetManager appWidgetManager, int[] appWidgetIds) {
		...
		for(int appWidgetId : appWidgetIds) {
			...
			if( sharedPreferences.getBoolean(MainActivity.getShowWhenLockedPreferenceKey(), true) ) {
	            ...
	            intent.setAction("com.github.jamsinclair.owcamera.LAUNCH_OPEN_CAMERA");
			...
			/*if( sharedPreferences.getBoolean(MainActivity.getShowWhenLockedPreferenceKey(), true) ) {
				views.setTextViewText(R.id.launch_open_camera, "OW Camera for Pebble (unlocked)");
			}
			else {
				views.setTextViewText(R.id.launch_open_camera, "OW Camera for Pebble (locked)");
			}*/
	...
	public void onReceive(Context context, Intent intent) {
	    ...
	    if (intent.getAction().equals("com.github.jamsinclair.owcamera.LAUNCH_OPEN_CAMERA")) {
	    	if( MyDebug.LOG )
