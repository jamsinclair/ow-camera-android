package com.github.jamsinclair.owcamera2.preview;

import com.github.jamsinclair.owcamera2.JavaImageFunctions;
import com.github.jamsinclair.owcamera2.JavaImageProcessing;
import com.github.jamsinclair.owcamera2.cameracontroller.RawImage;
//import com.github.jamsinclair.owcamera2.MainActivity;
import com.github.jamsinclair.owcamera2.MyDebug;
import com.github.jamsinclair.owcamera2.R;
import com.github.jamsinclair.owcamera2.TakePhoto;
import com.github.jamsinclair.owcamera2.ToastBoxer;
import com.github.jamsinclair.owcamera2.cameracontroller.CameraController;
import com.github.jamsinclair.owcamera2.cameracontroller.CameraController1;
import com.github.jamsinclair.owcamera2.cameracontroller.CameraController2;
...
import com.github.jamsinclair.owcamera2.cameracontroller.CameraControllerManager1;
import com.github.jamsinclair.owcamera2.cameracontroller.CameraControllerManager2;
import com.github.jamsinclair.owcamera2.preview.ApplicationInterface.NoFreeStorageException;
import com.github.jamsinclair.owcamera2.preview.camerasurface.CameraSurface;
import com.github.jamsinclair.owcamera2.preview.camerasurface.MySurfaceView;
import com.github.jamsinclair.owcamera2.preview.camerasurface.MyTextureView;
 ...
 *  supports much of the OW Camera 2 for Pebble logic and functionality). Communication to
...
public class Preview implements SurfaceHolder.Callback, TextureView.SurfaceTextureListener {
    ...
    private List<String> noise_reduction_modes; // n.b., this is for the Camera2 API setting, not for OW Camera 2 for Pebble's Noise Reduction photo mode
    ...
    public void getMeasureSpec(int [] spec, int widthSpec, int heightSpec) {
        ...
        if( result % 180 != 0 ) {
            ...
            // E.g., if the device orientation is in portrait, it might still be that OW Camera 2 for Pebble
    ...
    public void onSurfaceTextureSizeChanged(@NonNull SurfaceTexture texture, int width, int height) {
        ...
        if( camera_controller != null ) {
            ...
            if( set_preview_size && (width != preview_w || height != preview_h) ) {
                ...
                // Needed to fix problem if OW Camera 2 for Pebble is already running, and the aspect ratio changes (e.g.,
    ...
    public CameraController.Size getOptimalPreviewSize(List<CameraController.Size> sizes) {
        ...
        Point display_size = new Point();
        {
            ...
            // (a) it would be good to not assume OW Camera 2 for Pebble runs in landscape mode (if we ever ran in portrait mode,
     ...
     *  OW Camera 2 for Pebble always runs in landscape).
    ...
    public void showToast(final ToastBoxer clear_toast, final String message, final int offset_y_dp, final boolean use_fake_toast, boolean dont_clear) {
        ...
        if( this.app_is_paused && use_fake_toast ) {
            ...
            // is useful for cases where the toast is created shortly before OW Camera 2 for Pebble resumes, e.g., cancelling SAF
